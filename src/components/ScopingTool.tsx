    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="lg:col-span-2 space-y-4">
        <h3 className="text-xl font-bold flex items-center gap-2 mb-6">
          <Target className="text-indigo-400" /> Rapid Proposal Builder
        </h3>
          ...
          {SERVICES.map((service) => (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <span className={`text-sm font-bold uppercase tracking-widest ${selected.includes(service.id) ? 'text-indigo-300' : 'text-slate-500'}`}>
                  {service.name}
                </span>
      ...
      <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 flex flex-col h-fit sticky top-8">
        ...
        <div className="space-y-4 mb-10">
          <div className="flex items-center gap-3 text-xs text-emerald-400 font-bold uppercase tracking-tighter">
            <Shield size={14} /> Fixed Price Guarantee
          </div>
          <div className="text-xs text-slate-400 leading-relaxed">
            Includes all selected services with a 14-day delivery window and post-launch support.
          </div>
        </div>

        <button 
          onClick={() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }}
          className="w-full py-4 bg-white text-slate-950 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-400 hover:text-white transition-all mb-4"
        >
          {copied ? <><Check size={18} /> Copied to Clipboard</> : <><Copy size={18} /> Copy Proposal Link</>}
        </button>
        
        <button className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-500 transition-all">
          Generate PDF <ChevronRight size={18} />
        </button>
      </div>
    </div>
  );
